...
class SSFPN(nn.Module):
    def __init__(self, backbone, pretrained=True,classes=11):
        ...
        self.cfgb = nn.Sequential(
            conv_block(2048, 512, 3, 2, padding=1, group=512, dilation=1, bn_act=True),
            nn.Dropout(p=0.15))
        ...
        self.gfu4 = GlobalFeatureUpsample(256,256,256)
        self.gfu3 = GlobalFeatureUpsample(128,256,128)
        self.gfu2 = GlobalFeatureUpsample(64, 128,64)
        self.gfu1 = GlobalFeatureUpsample(32,64,32)
...
class PyrmidFusionNet(nn.Module):
    def __init__(self,channels_high,channels_low,channel_out,classes=11,groups=2):
...
class GlobalFeatureUpsample(nn.Module):
    def __init__(self,low_channels,in_channels,out_channels,red=1):
    ...
    def forward(self,x_gui,y_high):
        ...
        y_high= F.avg_pool2d(self.conv2(y_up),(1,1))
        out = y_high+x_gui
