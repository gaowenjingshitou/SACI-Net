...
class ModelTrainer(object):
    ...
    def train(data_loader, model, loss_f, optimizer, scheduler, epoch_idx, device, cfg, logger):
        ...
        for i, data in enumerate(data_loader):#data torch.Size([128, 3, 32, 32])。
           ...
           # label_list.extend(labels.tolist())
            ...
            if cfg.mixup:
                ...
                now_acc,loss = mixup_criterion(loss_f, outputs.cpu(), label_a.cpu(), label_b.cpu(), epoch_idx,lam)
            else:
          ...
          #  scaler.scale(loss).backward()
            ...
            acc_mixup.append(now_acc.item())
            acc_mixup_mean=np.mean(acc_mixup)
        ...
        #return loss_mean, acc_avg, acc_mixup_mean,conf_mat, path_error

        return loss_mean, acc_mixup_mean, conf_mat, path_error
