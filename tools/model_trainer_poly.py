...
from utils.lr_poly_scheduler  import WarmupPolyLR
...
class ModelTrainer(object):
    ...
    @staticmethod
    def train(args,data_loader, model, loss_f, optimizer, epoch_idx, device, cfg, logger):
        model.train()
        ...
        for i, data in enumerate(data_loader):#data torch.Size([128, 3, 32, 32])。
            ...
            args.cur_iter = epoch_idx * args.per_iter + i
          ...
          #  scaler.scale(loss).backward()
            ...
            if i % cfg.log_interval == cfg.log_interval - 1:
                logger.info("Training: Epoch[{:0>3}/{:0>3}] Iteration[{:0>3}/{:0>3}] Loss: {:.4f} Acc:{:.2%} lr:{}".
                            format(epoch_idx + 1, cfg.epochs, i + 1, len(data_loader), loss_mean, acc_avg,lr))
