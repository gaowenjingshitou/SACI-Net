...
class Combiner:
    ...
    def bbn_mix(self, model, criterion, image, label, meta, **kwargs):
        print("image",image)
        print("label", label)
        image_a=image.tensors.to(self.device)
        image_b = meta["sample_image"].to(self.device)

        
       # image_a, image_b = image.tensors.to(self.device), meta["sample_image"].to(self.device)
        ...
        loss = l * criterion(output, label_a,self.epoch) + (1 - l) * criterion(output, label_b,self.epoch)
